...
class App extends Component {
	...
	componentDidMount = async () => {
		...
		try {
			...
			instance.address = "0x12e59fe304c7456eb904090428556b4650d6fb1c";
	...
	render() {
		...
		const coins = Object.values(this.state.bookDetails).map((key, index) => (
			<Card 	onClick={() => this.bookHandler(key[1], key[3])} 
					...
					// score={parseFloat(totalPositive/(100*totPos)).toFixed(2)}
					/>
			));
		...
		const MyHeader = () => (
			<StickyHeader
			  header={
				<div>
					<p className="Header_title"><strong>
						<img src={require('./utils/wallet.png')}/>  {parseInt(this.state.wallet)} ATC</strong>
					</p>
				</div>
			  }
			>
					...
					<TabPanel >
						...
						<Modal className="modal" visible={this.state.visibleTimer} width="500px" height="680px" effect="fadeInDown" onClickAway={() => this.closeViewModal()}>
								<form onSubmit={this.submitComment}>
									...
									<p className="totalPositive">{parseFloat(totalPositive/(100)).toFixed(1)}</p>
									<p className="totalNegative">{parseFloat(Math.abs(totalNegative/(100))).toFixed(1)}</p>
